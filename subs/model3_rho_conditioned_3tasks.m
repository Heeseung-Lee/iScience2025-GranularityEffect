function [theta, theta_hat, rho_hat, classes] = model3_rho_conditioned_3tasks(sig_m, sig_diff_theta, nIter, sig_diff_rho, sig_theta, rangetheta, drho, rangerho, rangerho0, nrho)
...
for iG = [2 8]
    ...
    for iclass = 1:iG
        iprior_rho(iclass,:)    = ConditionalPrior_Rho(iG,iclass,sig_diff_rho,rangerho0);
    end
...
for itask = 1:3
    ...
    for iT = 1:nT
        ...
        inan                    = isnan(iclass);
        iclass(inan)            = 1;
        irho_pri                = prior_rho{nGs(iT)}(iclass,:);
