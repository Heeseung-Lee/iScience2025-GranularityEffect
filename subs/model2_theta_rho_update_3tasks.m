function [theta, theta_hat, rho_hat, classes] = model2_theta_rho_update_3tasks(sig_m, sig_diff_theta, nIter, sig_theta, rangetheta, drho, rangerho, nrho, sig_diff_rho, rangerho0)

nT  = 2; % only two trials

% stimulus sample is shared by the simulations
theta           = normrnd(0,sig_theta,[2 nIter]);
...
for itask = 1:3
    ...
    for iT = 1:nT
        ...
        [iclass, irho_hat, irho_pos]    = rho_inference(nGs(iT), im(iT,:), sig_m, imu0, isig0, rangetheta, rangerho, drho, irho_pri, rhobounds{iT});
